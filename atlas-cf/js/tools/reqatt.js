...
function _raRequiredAbbrs(category, attachmentTypes) {
 ...
 */
function _raEvaluate(category, attachmentTypes, origin) {
  const required = [];
  for (const abbr of _raRequiredAbbrs(category, attachmentTypes)) {
    let label, satisfied;
    if (abbr === "AWB") {
      label = "AWB/BoL";
...
function reqattRun(grid, startIso, endIso) {
  ...
  for (const blk of blocks) {
    ...
    if (shipKey === "hand carry" || shipKey === "usps") continue;
    ...
    const { missing } = _raEvaluate(category, blk.attachment_types, origin);
...
function reqattAuditSrf(grid) {
  ...
  const shipKey = shipmentAs.toLowerCase().replace(/[-_]/g, " ").split(/\s+/).filter(Boolean).join(" ");
  const handOrUsps = shipKey === "hand carry" || shipKey === "usps";

  const classifiable = !!(origin && dest);
  const category = classifiable ? _raShipmentCategory(origin, dest) : "";

  let required = [], missing = [];
  if (classifiable && !handOrUsps) {
    const ev = _raEvaluate(category, attachment_types, origin);
  ...
  return {
    ...
    hand_or_usps: handOrUsps, classifiable, category,
...
const ReqAttUi = { result: null, start: "", end: "", showCats: false };
...
function renderReqattWorkspace(container) {
    ...
    <div class="panel">
      ...
      <div class="body">
        <div class="note">
          ...
          Hand Carry and USPS shipments are skipped. Lists only WMTRs that are <strong>missing</strong> one or more
          required documents.
        </div>
  ...
  g("raRun").addEventListener("click", runReqatt);
  g("raAllTime").addEventListener("click", () => { g("raPeriod").value = "Show All"; runReqatt(); });
...
function runReqatt() {
  ...
  try {
    ...
    if (result.missing_country_rows.length) {
      const preview = result.missing_country_rows.slice(0, 10).map((r) => `${r[0]}: ${r[1]}`).join(" · ");
      const more = result.missing_country_rows.length - Math.min(result.missing_country_rows.length, 10);
      warn.textContent = `${result.missing_country_rows.length} delivered WMTR(s) couldn't be classified (blank country): ` +
...
function renderReqattTable(result) {
  ...
  if (!result.rows.length) {
    ...
    tbody.innerHTML = result.rows.map((r) =>
      `<tr>${r.map((v) => `<td>${esc(v == null ? "" : String(v))}</td>`).join("")}</tr>`).join("");
  }
  wrap.style.display = "";
}
...
function reqattSrfSummaryLines(a) {
  ...
  if (a.hand_or_usps) { lines.push("Hand Carry / USPS — no attachment check required."); return lines; }
...
function exportReqattSrf() {
  ...
  try {
    ...
    const missingText = !a.classifiable ? "(unclassified — missing country)"
      : a.hand_or_usps ? "(no check required — Hand Carry/USPS)"
